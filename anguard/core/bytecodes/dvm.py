...
from anguard.core import bytecode, anconf
from anguard.core.bytecodes.apk import APK
from anguard.core.anconf import CONF, debug, warning
...
class EncodedValue(object):
    ...
    def __init__(self, buff, cm):
        ...
        if self.value_type >= VALUE_SHORT and self.value_type < VALUE_STRING:
            ...
            anconf.warning("Unknown value 0x%x" % self.value_type)
...
class MapItem(object):
    ...
    def next(self, buff, cm):
        anconf.debug("Parsing section %s" % TYPE_MAP_ITEM[self.type])
        ...
        if TYPE_MAP_ITEM[self.type] == "TYPE_STRING_ID_ITEM":
            ...
            anconf.warning("Map item %d @ 0x%x(%d) is unknown" %
                          (self.type, buff.get_idx(), buff.get_idx()))

        diff = time.time() - started_at
        minutes, seconds = float(diff // 60), float(diff % 60)
        anconf.debug("End of parsing %s = %s:%s" % (TYPE_MAP_ITEM[self.type], str(minutes), str(round(seconds,2))))
...
class ClassManager(object):
    ...
    def get_class_data_item(self, off):
        ...
        anconf.warning("unknown class data item @ 0x%x" % off)
...
class MapList(object):
    ...
    def __init__(self, cm, off, buff):
        ...
        for i in self.map_item:
            anconf.debug("Reloading %s" % TYPE_MAP_ITEM[i.get_type()])
            started_at = time.time()
            i.reload()
            diff = time.time() - started_at
            minutes, seconds = float(diff // 60), float(diff % 60)
            anconf.debug("End of reloading %s = %s:%s" % (TYPE_MAP_ITEM[i.get_type()], str(minutes), str(round(seconds,2))))
