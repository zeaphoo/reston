...
from anguard.core import bytecode
from anguard.core import anconf
from anguard.core.bytecodes.dvm_permissions import DVM_PERMISSIONS
from anguard.util import read

from anguard.core.resources import public
...
def sign_apk(filename, keystore, storepass):
    ...
    compile = Popen([anconf.CONF["PATH_JARSIGNER"], "-sigalg", "MD5withRSA",
...
class APK(object):
    ...
    def __init__(self,
        ...
        if zipmodule == 0:
            ...
            from anguard.core import zipfile
    ...
    def get_app_icon(self, max_dpi=65536):
        ...
        if app_icon.startswith("@"):
            ...
            try:
                ...
                anconf.warning("Exception selecting app icon: %s" % e)
    ...
    def _patch_magic(self, buffer, orig):
        if ("Zip" in orig) or ("DBase" in orig):
            val = anconf.is_android_raw(buffer)
            if val == "APK":
    ...
    def new_zip(self, filename, deleted_files=None, new_files={}):
        ...
        """
        if self.zipmodule == 2:
            from anguard.patch import zipfile
...
class StringBlock(object):
    ...
    def skipNullPadding(self, buff):
        ...
        def readNext(buff, first_run=True):
            ...
            if header == CHUNK_NULL_TYPE and first_run:
                anconf.info("Skipping null padding in StringBlock header")
                header = readNext(buff, first_run=False)
            elif header != CHUNK_STRINGPOOL_TYPE:
                anconf.warning("Invalid StringBlock header")
    ...
    def decode_bytes(self, data, encoding, str_len):
        ...
        if len(string) != str_len:
            anconf.warning("invalid decoded string length")
        return string
...
class AXMLParser(object):
    ...
    def __init__(self, raw_buff):
        ...
        if axml_file == CHUNK_AXML_FILE:
            ...
            anconf.warning("Not a valid xml file")
        ...
        while True:
            ...
            if chunkType == CHUNK_RESOURCEIDS:
                ...
                if chunkSize < 8 or chunkSize % 4 != 0:
                    anconf.warning("Invalid chunk size")
            ...
            if chunkType < CHUNK_XML_FIRST or chunkType > CHUNK_XML_LAST:
                anconf.warning("invalid chunk type")
    ...
    def getAttributeOffset(self, index):
        ...
        if self.m_event != START_TAG:
            anconf.warning("Current event is not START_TAG.")

        offset = index * 5
        # FIXME
        if offset >= len(self.m_attributes):
            anconf.warning("Invalid attribute index")
...
def format_value(_type, _data, lookup_string=lambda ix: "<string>"):
    if _type == TYPE_STRING:
        ...
        return "%d" % anconf.long2int(_data)
...
class ARSCParser(object):
    ...
    def get_resource_dimen(self, ate):
        try:
            ...
            anconf.debug("Out of range dimension unit index for %s: %s" % (
                complexToFloat(ate.key.get_data()),
    ...
    def __init__(self, buff=None, **kwargs):
        if buff is not None:
            ...
            if self.exceedingSize > 0:
                anconf.info("Skipping padding bytes!")
