...
import reston.core.anconf as anconf
import reston.decompiler.dad.util as util
from reston.core import analysis
from reston.core import apk, dvm
from reston.decompiler.dad.ast import (
    ...
    dummy)
from reston.decompiler.dad.control_flow import identify_structures
from reston.decompiler.dad.dataflow import (
    build_def_use, place_declarations, dead_code_elimination,
    register_propagation, split_variables)
from reston.decompiler.dad.graph import construct, simplify, split_if_nodes
from reston.decompiler.dad.instruction import Param, ThisParam
from reston.decompiler.dad.writer import Writer
...
def auto_vm(filename):
    ret = anconf.is_android(filename)
    if ret == 'APK':
...
def main():
    ...
    default_file = 'examples/android/TestsReston/bin/TestActivity.apk'
    if len(sys.argv) > 1:
